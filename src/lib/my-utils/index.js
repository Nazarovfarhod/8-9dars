...
export const collectStatistics = (array, type) => {
  const result = [];
  const data = {};
  for (const obj of array) {
    if (data[obj[type]] == 1) data[obj[type]] += 1;
    else data[obj[type]] = 1;
  }

  for (const key in data) {
    result.push({ key, qiymat: data[key] });
  }
...
export const myTranslation = (word) => {
  switch (word) {
    case "category":
      return "Turkum bo'yicha";
    case "country":
      return "Hudud bo'yicha";
    case "color":
      return "Rangi bo'yicha";
    default:
